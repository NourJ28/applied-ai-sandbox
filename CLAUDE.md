...
- If Claude proposes editing a test to "make it pass," push back. The
  tests are the spec.
## Auth
- Use Flask-Login + werkzeug.security for all auth flows.
- Never roll a custom session scheme; never store plaintext passwords.
- Password reset, remember-me, and email verification are out of scope — do not add them unless explicitly asked.
